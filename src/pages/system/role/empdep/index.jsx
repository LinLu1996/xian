...
class Empdep extends Component {
  ...
  componentDidMount() {
    const me = this;
    //角色默认已拥有资源
    IO.role_res.gethavedata({':id': me.props.emprole_.id}).then((res) => {
      this.setState({
  ...
  emporgChange() {
    ...
    IO.role_res.getdata({roleId: this.props.emprole_.id, resIds: JSON.stringify(arrres)}).then((res) => {
      if (res.success) {
    ...
    return (
      <div className="allinfobox-j">
        ...
        <div className='system-title-div'>
          <span className='system-title'>角色：{this.props.emprole_.roleName}</span>
        </div>
